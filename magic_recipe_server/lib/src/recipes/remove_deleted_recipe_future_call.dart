...
/// This future call is used to reschedule the removal of deleted recipes every 5 minutes.
///
/// This is useful to clean up the database and remove any recipes that were deleted by the user.

class ReschedulingRemoveDeletedRecipesFutureCall extends FutureCall {
  @override
  Future<void> invoke(Session session, SerializableModel? _) async {
    // We can run the future call with the logic directly, no need to schedule it
    await RemoveDeletedRecipesFutureCall().invoke(session, null);

    final pod = session.serverpod;
    // Cancel any existing future call with this name to avoid scheduling multiple future calls
